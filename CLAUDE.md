...
`public/library/scratch-catalog.json` is checked in and regenerated by `node scripts/build-scratch-catalog.ts`, which pins a `scratch-gui` commit SHA for reproducibility and downloads metadata only. Scratch media is CC BY-SA 4.0 — see `public/library/LICENSE.md`.
